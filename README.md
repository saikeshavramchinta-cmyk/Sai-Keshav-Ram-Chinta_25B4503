...
A model is only as good as the data you feed it. Instead of just looking at raw prices, I engineered features that capture both the *direction* and the *uncertainty* of the market:
*   **Momentum:** Calculated using rolling windows to see if the macro trend is pointing up or down. 
*   **Volatility:** Calculated using the rolling standard deviation of returns to measure how wildly prices are swinging.
*   **Indian VIX:** A natural, forward-looking proxy for market fear.

### 3. Letting Stats Drive the Weights (Convex Optimization)
Instead of guessing how much of our money should be in stocks versus bonds, I used **convex optimization** to mathematically pick the best portfolio weights for whatever regime we are currently in. 
*   In a **Bull** market, the math optimizes for maximum return. 
*   In a **Crisis**, the objective function flips entirely to minimize volatility, automatically forcing a flight to safe assets. 

### 4. Keeping the Math Honest (No Time-Traveling!)
The easiest trap to fall into when building financial models is **lookahead bias**—accidentally letting the model peek at future data. To prevent the model from "cheating," I built a strict **walk-forward validation harness**. The HMM is re-fit only using past data at every single step, moving forward through time. This ensures our testing is grounded in reality.

### 5. Staying Grounded in Reality (Transaction Costs)
Trading isn't free. If a model rapidly flips between assets every two days, it might look great on paper, but the transaction costs will quietly destroy your real-world returns. To reflect reality, I explicitly modeled a **5-10 basis point transaction cost** for every rebalance. 

---

## 🛠️ The Tech Stack
*   **Python 3.9+**
*   **yFinance:** To pull the daily returns for our assets and the VIX.
*   **hmmlearn:** The statistical brains behind detecting the hidden market regimes.
*   **CVXPY:** The optimization engine that solves for our exact portfolio weights.
*   **Pandas, NumPy, & Matplotlib:** For wrangling data and generating our visualizations.
...
The Regime Chart: A plot of the historical price data with colored bands layered on top, showing exactly when the HMM thought we were in a Bull, Bear, or Crisis market.

The Transition Matrix: A matrix showing the probability of the market switching from one state to another (proving that market regimes tend to be "sticky").

The Scorecard: A performance summary comparing our smart dynamic strategy against a static 60/40 portfolio and an equal-weight portfolio. We compare them fairly using the Sharpe ratio, Sortino ratio, max drawdown, Calmar ratio, and turnover.
